...
- [ ] 1.1 Create Django project (`osint_platform`) with standard layout: apps/, templates/, static/, manage.py
- [ ] 1.2 Configure settings: PostgreSQL, AWS S3 via django-storages, HTMX, Tailwind CSS (standalone CLI)
- [ ] 1.3 Create `render.yaml` Blueprint (web service + PostgreSQL)
- [ ] 1.4 Create Dockerfile + docker-compose.yml for local development (Django + PostgreSQL)
- [ ] 1.5 Set up initial migration and create superuser command

## 2. Multi-Tenancy: Models & Middleware

- [ ] 2.1 Create `core` app with Organization model (name, slug, description)
- [ ] 2.2 Create User model (extend AbstractUser, email as username)
- [ ] 2.3 Create OrganizationMembership model (user, organization, role: owner/admin/member)
- [ ] 2.4 Create Project model (organization FK, name, slug, description) with org-scoped uniqueness
- [ ] 2.5 Create ProjectMembership model (user, project, role: coordinator/volunteer)
- [ ] 2.6 Write OrgProjectMiddleware: resolve org + project from URL `/{org-slug}/{project-slug}/`, attach to `request.organization` and `request.project`, enforce membership
- [ ] 2.7 Create OrgScopedManager — custom queryset that auto-filters by `request.organization`
- [ ] 2.8 Write permission mixins: `RequireOrgRole(role)`, `RequireProjectRole(role)`
...
- [ ] 3.6 Invitation model (email, org, project, role, token, expires_at) + send invite email
...
- [ ] 4.1 Create Platform model (project FK, name, url_pattern for auto-detection)
- [ ] 4.2 Create Language model (project FK, name, code)
- [ ] 4.3 Create KeywordCategory model (project FK, name, slug)
- [ ] 4.4 Create ProjectFieldConfig model (project FK, field_name, label, type, required, choices JSON, order)
- [ ] 4.5 Create ProjectSettings model (project FK, record_id_prefix)
...
- [ ] 5.1 Create `incidents` app
- [ ] 5.2 Create Incident model: core fields (record_id, platform FK, url, screenshot FileField, date_of_post, date_collected, collected_by FK, location_mentioned, probable_location, language, confidence, notes, status) + extra_fields JSONField + organization FK + project FK
- [ ] 5.3 Auto-generate record_id on save using project's prefix + sequential counter
...
- [ ] 5.7 Duplicate detection: on save, check for existing incident with same URL in project, flag as potential duplicate
...
- [ ] 7.1 Create `keywords` app
- [ ] 7.2 Create Keyword model (project FK, organization FK, term, language, category FK, platform_relevance JSON, added_by FK, date_added, status, shared boolean)
...
- [ ] 8.1 Create `coverage` app
- [ ] 8.2 Create SearchSession model (project FK, volunteer FK, platform FK, language, keyword_categories JSON, date, duration_minutes, incidents_found, notes)
